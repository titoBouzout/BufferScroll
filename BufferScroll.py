...
class Pref:
    def load(self):
        ...
        Pref.typewriter_scrolling_follow_cursor_movement = s.get(
...
class BufferScroll(sublime_plugin.EventListener):
    ...
    def on_deactivated(self, view):
        ...
        last_focused_view_name = (
            ...
            + str(view.file_name())
            + "-"
            + str(view.settings().get("is_widget"))
        )
    ...
    def save(self, view, where="unknow"):
        ...
        if view.is_loading():
            sublime.set_timeout(lambda: self.save(view, where), 0)
        else:
    ...
    def view_id(self, view):
        if not view.settings().has("buffer_scroll_name"):
            view.settings().set(
                "buffer_scroll_name",
                sha1(normpath(str(view.file_name()).encode("utf-8"))).hexdigest()[:8],
            )
    ...
    def restore_scroll(self, view, where="unknow"):
        ...
        if view.is_loading():
            sublime.set_timeout(lambda: self.restore_scroll(view, where), 0)
        else:
            scroll_already_restored[view.id()] = True
            if (
                last_focused_view_name == "-None-None"
                ...
                if id in db and Pref.get("restore_scroll", view):
                    ...
                    print_debug(
                        "supposed current scroll: " + str(view.viewport_position())
                    )
                    # THIS LIES
                else:
                    print_debug("SKIPPED...")
    ...
    def restore(self, view, where="unknow"):
        ...
        if view.is_loading():
            sublime.set_timeout(lambda: self.restore(view, where), 0)
        else:
            ...
            if id in db:
                ...
                # color scheme
                if (
                    Pref.get("remember_color_scheme", view)
                    and "c" in db[id]
                    and view.settings().get("color_scheme") != db[id]["c"]
                ):
                ...
                # syntax
                if (
                    Pref.get("remember_syntax", view)
                    and "x" in db[id]
                    and view.settings().get("syntax") != db[id]["x"]
                    and lexists(dirname(sublime.packages_path()) + "/" + db[id]["x"])
                ):
                ...
                # scroll
                if (
                    Pref.get("restore_scroll", view)
                    and Pref.get("i_use_cloned_views", view)
                    and index in db[id]["l"]
                ):
                    position = tuple(db[id]["l"][index])
                    view.set_viewport_position(position, Pref.use_animations)
                ...
                print_debug("scroll set: " + str(position))
                print_debug("supposed current scroll: " + str(view.viewport_position()))
                # THIS LIES
            else:
                print_debug("SKIPPED...")
    ...
    def synch_data(self, view=None, where="unknow"):
        ...
        if (
            not Pref.get("synch_bookmarks", view)
            and not Pref.get("synch_marks", view)
            and not Pref.get("synch_folds", view)
        ):
            return
        ...
        if view.is_loading():
            Pref.synch_data_running = False
            sublime.set_timeout(lambda: self.synch_data(view, where), 0)
        else:
    ...
    def synch_scroll(self):
        ...
        if Pref.synch_scroll_last_view_id != Pref.current_view_id:
            ...
            Pref.synch_scroll_last_view_position = 0
        last_view_position = str(
            [view.visible_region(), view.viewport_position(), view.viewport_extent()]
        )
            ...
            if id in db:
                if "pf" in db[id]:
                    ...
                    view.set_viewport_position(
                        (position[0] - 1, position[1] - 1), Pref.use_animations
                    )
                    view.set_viewport_position(position, Pref.use_animations)
...
class BufferScrollListener(sublime_plugin.EventListener):
    def on_text_command(self, view, command_name, args):
        ...
        if (
            command_name == "goto_definition"
            or command_name == "navigate_to_definition"
        ):
    ...
    def on_post_text_command(self, view, command_name, args):
        ...
        if (command_name == "move" or command_name == "move_to") and Pref.get(
            "typewriter_scrolling_follow_cursor_movement", view
        ):
